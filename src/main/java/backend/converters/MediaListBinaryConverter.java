...
/**
 * Converts {@link MediaList} from/into binary format.
 */
	...
	public SeekableByteChannel convert(@Nonnull MediaList input) {
		...
		BsonDocument media = new BsonDocument(
			input
				...
				.entrySet()
				.stream()
				.map(entry -> new BsonElement(entry.getKey().toString(), new BsonString(entry.getValue())))
				.toList()
		);
			...
			return new MediaList(
				media
					...
					.reduce(
						new HashMap<>(),
						(map, entry) -> {
							map.put(UUID.fromString(entry.getKey()), entry.getValue().asString().getValue());
							return map;
						},
						(map1, map2) -> {
							map1.putAll(map2);
